...
Camera myCamera(glm::vec3(0, 0, 20.0f), glm::radians(0.0f), glm::radians(-90.0f), glm::vec3(0,1.0f,0));
...
void mouse_callback(GLFWwindow* window, double xPos, double yPos){
    ...
    myCamera.ProcessMouseMovement(deltaX,deltaY, GL_TRUE);
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods){
    ...
    if(glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS){
        mode = -1;
        myCamera.ProcessViewMovement(mode);
//        myCamera.UpdateCameraPos();
//        cout << key << GLFW_KEY_LEFT << endl;
    }else if (glfwGetKey(window, GLFW_KEY_RIGHT)==GLFW_PRESS){
        mode = 1;
        myCamera.ProcessViewMovement(mode);
//        myCamera.UpdateCameraPos();
//        cout << key << GLFW_KEY_RIGHT << endl;
    }
    ...
    //    }
    //    myCamera.UpdateCameraPos();
    //    cout << key << GLFW_KEY_LEFT
}

void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
    myCamera.speedZ = float(yoffset);
    myCamera.UpdateCameraPos();
}
